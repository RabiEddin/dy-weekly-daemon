...
# 2) 사이트 → docs/ (PDF 제외 동기화, GitHub Pages가 이 폴더를 서빙)
mkdir -p "$DOCS"
rsync -a --delete --exclude='vol-*.pdf' "$BUILD/" "$DOCS/"
...
for f in render_newspaper_md.py sync_stickers.py badge_server.py compress_pdf.py serve.sh publish.sh; do
